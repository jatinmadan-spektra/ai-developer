...
namespace BlazorAI.Components.Pages
{
    public partial class MultiAgent
    {
        ...
        // DisableHtml() added: without it, if an agent emits raw unfenced HTML tags
        // (instead of keeping everything inside a ```html fenced code block), Markdig
        // passes that raw HTML straight through and the browser renders it as LIVE DOM
        // elements floating in the middle of the chat message - which is exactly the
        // oversized, overlapping calculator UI showing up "out of the box" instead of
        // as a code block. DisableHtml() forces ALL raw HTML in agent output to be
        // escaped and shown as visible text instead of executed. Fenced code blocks
        // (```html ... ```) are unaffected and still render as proper syntax-highlighted
        // code either way.
